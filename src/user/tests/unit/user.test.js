...
const JWT_EXPIRATION = require(path.resolve('./config/vars')).jwtExpirationInterval;
...
describe('Users Module API', async () => {
  ...
  /**
   * Temp test data before each test
   */
  beforeEach(async () => {
    dbUsers = {
      keviveks: {
   ...
   */
  afterEach(async () => {
    // Makes the user collection empty
    await User.remove({});
  });
});
